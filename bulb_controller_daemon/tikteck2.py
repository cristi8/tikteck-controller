...
#
# Edited by Cristian Balas (GitHub username: cristi8):
# Code originally from https://github.com/mjg59/python-tikteck
# Adapted to python 2 and minor change to not wait for responses when sending packets
#
...
def encrypt_packet(sk, mac, packet):
    ...
    temp_buffer = bytearray(temp_buffer)
    for i in range(15):
    ...
    for i in range(15):
        if i == 0:
          temp_buffer2 = bytearray(   encrypt(sk, temp_buffer)   )
  ...
  def send_packet(self, msgid, command, data):
    ...
    while True:
      if time.time() - initial >= 10:
        return False
      try:
        response = self.device.writeCharacteristic(0x15, bytes(enc_packet), withResponse=False)
        break
